...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const sum2 = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  const cos = sum1 / sum2;
...
function roundToPowerOfTen(num, pow) {
  const step1 = num % 10 ** pow;
  const step2 = num - step1;
  const step3 = step1 / 10 ** pow;
  const step4 = Math.round(step3) * 10 ** pow;
  const result = step2 + step4;
  return result;
}
...
function getSumOfNumbers(x1, x2, x3) {
  return (Math.round(x1 * 10) + Math.round(x2 * 10) + Math.round(x3 * 10)) / 10;
}
/**
